...
class HNCacheStorageTestCase: XCTestCase {
    ...
    func testInsertTwoObjectsSuccessfully() {
        ...
        do {
            ...
            try HNStorage<HNCacheStorage>().insert(test2)
            try HNStorage<HNCacheStorage>().insert(test1)
        } catch {
            XCTAssert(true, error.localizedDescription)
        }
        
        let o_test1: TestClass? = try? HNStorage<HNCacheStorage>().get("1")
        let o_test2: TestClass? = try? HNStorage<HNCacheStorage>().get("2")
    ...
    func testUpdateAnObject() {
        ...
        test1.name = "test2"
        do {
            try HNStorage<HNCacheStorage>().update(test1)
        } catch {
            XCTAssert(true, error.localizedDescription)
        }

        let o_test2: TestClass? = try? HNStorage<HNCacheStorage>().get("1")
    ...
    func testDeleteAnObject() {
        ...
        do {
            try HNStorage<HNCacheStorage>().delete(test1)
        } catch {
            XCTAssert(true, error.localizedDescription)
        }
        
        
        let o_test1_: TestClass? = try? HNStorage<HNCacheStorage>().get("1")
        let o_test2_: TestClass? = try? HNStorage<HNCacheStorage>().get("2")
    ...
    func testInsertObjectWithNamedUserDefaults() {
        ...
        try? HNStorage<HNCacheStorage>().delete(test2)
        try? HNStorage<HNCacheStorage>(id: userDefaultsSuitName).delete(test1)

        //insert
        do {
            try HNStorage<HNCacheStorage>(id: userDefaultsSuitName).insert(test1)
            try HNStorage<HNCacheStorage>().insert(test2)
        } catch {
            XCTAssert(true, error.localizedDescription)
        }
        ...
        //get
        let o_test1: TestClass? = try? HNStorage<HNCacheStorage>().get("2")
        let o_test2: TestClass? = try? HNStorage<HNCacheStorage>(id: userDefaultsSuitName).get("1")
